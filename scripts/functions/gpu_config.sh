...
intel_gpu_config() {
  ...
  if lspci | grep -q "Intel.*Graphics"; then
    ...
    if ! lsmod | grep -q "intel_iommu"; then
      ...
      run_command "sed -i '/GRUB_CMDLINE_LINUX_DEFAULT=/s/\"$/ intel_iommu=on iommu=pt pcie_acs_override=downstream,multifunction nofb nomodeset video=vesafb:off,efifb:off\"/' /etc/default/grub"
      run_command "update-grub"
    fi

    # Install necessary drivers
    if ! command -v intel-media-va-driver &> /dev/null; then
      msg_info "Installing Intel Media VA Driver"
      run_command "apt update" || return 1
      run_command "apt install -y intel-media-va-driver" || return 1
    fi
